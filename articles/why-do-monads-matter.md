...
파괴적인 세계에서의 컴퓨터 프로그래밍 함수는 매우 중요하다. 이 특징으로 인해 프로그래밍에서 에러가 더 쉽게 발생하며,  작업을 나누어서 동시에 실행하는 것이 더 어려워진다. 예를들어, 현대적인 멀티코어 컴퓨터를 갖고 있다면 나누어진 작업들을 동시에 실행하고 싶겠지만, 작업들의 실행순서가 잘못되면 정상적으로 동작하지 않을지도 모른다. 하지만 동시에 이 파괴적인 효과는 컴퓨터 프로그래밍에서 가장 중요한 관념이다. 프로그래밍에서 함수의 파괴적인 결과를 관찰할 수 없다면 구동할 의미가 없기 때문이다. (역주: 프로그램이 화면에 아무것도 뜨지않고 계산만 한다고 생각해보자.) 그래서 실질적으로 모든 주류 프로그래밍 언어에서의 함수는 파괴성의 문제를 잘 다룰 필요가 있다.

## 함수로 돌아와서
컴퓨터 프로그래밍 세계에서 발생하는 여러 문제를 찾아 직접 확인했다. 우리는 실패할지도 모르고, 의존성으로 인한 외부의 맥락을 해결해야하고, 비결정론적 선택을 하며, 연산을 수행할 때 가끔씩 관찰가능할 효과가 발생할 수도 있는 소프트웨어를 개발한다.
