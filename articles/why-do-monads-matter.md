...
- 이 범주에 있는 화살표는, 놀랍지 않게도, Kleisli 화살표(함수)이다.
- 아직 항등과 합성에 대해 다루지 않았다. 아래에서 살펴보도록 하자.

먼저 **실패**를 살펴보자. 정의역 _A_에서 치역 _B_를 갖는 실패 Kleisli 함수, 그리고 정의역 _B_에서 치역 _C_을 갖는 실패 Kleisli 함수가 주어졌다. 이 두 화살표를 정의역 _A_에서 치역 _C_을 갖는 Kleisli 함수로 합성하려 한다. 다시 말하면, 정의역 _A_ 치역 _Err(B)_의 함수, 정의역 _B_와 치역 _Err(C)_의 함수, 이 평범한 두 함수를 합성해서 정의역 _A_ 치역 _Err(C)_를 갖는 함수를 얻으려고 하는 것이다. 무엇을 해야할지 잠시 생각해보자.
...
범주의 정의를 끝내기 위해서는 실패 Kleisli _항등함수_도 결정할 필요가 있다. 이 함수는 아무 일도 하지 않는 것이다. 그래서 다른 어떤 종류의 Kleisli 함수와 합성을 한다고 해도, 다른 함수에 영향을 주지 않는다. 항등성을 정의역 _A_, 치역 _Err(A)_ 함수로 생각하면, 마치 평범한 집합과 같이 함수 _f(x) = x_ 에 불과하다. 그 뜻은 절대 오류를 반환하지 않는다는 것 즉, 결과를 성공적으로 반환한다는 뜻이다.

남은 세가지 예를 좀 더 자세하게 설명하려고 한다. 이게 어떤 방식으로 동작하는지 아직 명확하게 이해가 되지 않을 수 있다. Kleisli 함수의 범주가 어떤 것인지 정의를 이해하는데 도움이 되도록 차근차근 살펴보려고 하니 희망을 잃지 말자.

다음은 **의존성**을 위한 Klesli 함수 즉, 정의역 _A_와 치역 _Pref(B)_를 갖는 함수를 살펴보자. _Pref_를 치역에서 사용하기 위해 호출하는 것은 어플리케이션 설정을 위해 새로운 매개변수를 추가하는 것과 동일하다. 가장 중요한 아이디어는 다음과 같다. 만약 어플리케이션 설정을 필요로 하는 두 함수가 있다고 한다면, 두 함수 모두에게 어플리케이션 설정을 줘야만 할 것이다. 두 의존성 Kleisli 함수를 합성해서 한 개의 새로운 함수를 만들면, 이 함수는 어플리케이션 설정을 추가적인 매개변수로 받아 내부의 두 콤포넌트_(역주: 합성된 두 함수)_를 따라 순차적으로 넘겨준다. 의존성 Kleisli 항등함수는 추가적인 설정 매개변수를 받지만 그것은 무시하고 받은 입력값을 그대로 반환할 것이다.

**불확실성**, 또는 비결정론을 위한 Kleisli 함수는 정의역 _A_와 치역 _P(B)_를 갖는 함수로 _P(B)_는 _B_의 멱집합이다. 비결정론에서의 주요 아이디어는 각각의 단계에서, 현재 가능한 모든 값을 시도해 모든 결과를 수집하는 것이다. 그래서 두 불확실성 Kleisli 함수를 함성한 함수가 있다면, 첫 함수에서 나온 모든 결과들을 각각 두 번째 함수에 넣고, 거기서 나온 모든 결과들을 합집합으로 묶어서 결과로 돌려준다. 물론 불확실성 Kleisli 항등함수는 실제로 비결정론적인 것은 아니며, 입력값을 하나의 원소만을 가진 집합으로 바꿔서 반환하게 된다.

마지막으로 **파괴적인 효과**를 위한 Kleisli 함수 즉, 정의역 _A_, 치역 _IO(B)_를 갖는 함수다. 여기서 중요한 아이디어는 합성된 함수의 순서대로 그 명령어 리스트를 조합하는 것이다. _(역주: 역시 명령어는 실행되지 않으며 명령어 리스트의 조합만 한다.)_ 이 두 함수의 합성하면 명령어의 순서는 다음과 같다. 첫 번째 함수의 명령어 리스트를 실행한다. 첫 번째 함수에서 나온 결과를 두 번째 함수에 넘겨준다. 마지막으로 두 번째 함수의 명령어 리스트를 실행한다. 여기서 이 Kleisli의 항등함수는 지시는 아무 행동도 하지 않고 입력이 그대로 결과로 전달된다. 여기까지 4가지의 흥미로운 예제를 통해 새로운 범주, 즉 Kleisli 범주를 만들었다.

이 새로운 범주는 함수와 같이 행동하고, 합성이나 항등함수와 같은 연관 개념을 가지고 있도록 만들어서 각각 특정 문제의 특별한 성질을 표현할 수 있게 되었다. Kleisli 범주를 활용해 만든 합성에 대한 관념은 오랫동안 컴퓨터 프로그래밍에서 문제였던 부분을 멋지고 조화롭게 해결할 수 있게 한다.
