...
### 함수로 의존성 다루기
두번째로 작성할 의사 함수는 함수 외부에 있는 세계에서 의존적인 정보를 얻는 경우다. 예를 들어 환경설정이나 어플리케이션 설정 등의 정보에서 말이다. 앞에서 작성한 방법과 비슷한 요령으로, 집합 _A_에 대해 _Pref(A)_는 함수들의 집합으로써, 각 함수들의 정의역은 어플리케이션의 설정이며 치역은 _A_ 집합이다. 이제 가까이 보면, 정의역이 _A_이고 치역이 집합 _B_인 함수에서 정의역이 _A_이고 치역이 집합 _Pref(B)_인 평범한 함수가 된다. 다시 말하면, 집합 _A_로부터 값을 받아, 다른 함수에서 어플리케이션 설정을 집합의 요소 하나하나에 적용(map)한 집합 _B_를 얻게 되는 것이다.

복잡하게 들릴지도 모르지만, 함수를 치역으로 가지고 있고 매개변수 두 개를 가진 함수일 뿐이다. 단, 두 개의 매개변수는 한번에 하나씩만 입력받는다. 시간을 들여 이 내용을 스스로 이해해보도록 하자. 이런 변환은 때로 "커링(currying)" 이라고 불린다. 이렇게 치역을 바꾸는 것은 사실상 새로운 매개변수를 추가하는 효과를 갖고, 어플리케이션 설정을 매개변수를 통해 받는 것과 같게 된다. 의존적 정보를 처리하기 위해서는 이 방법이 분명 우리가 원하는 것이지만 매개변수를 추가하는 것은 더욱 불편한 방향이라는 것도 기억하자 &mdash; 나중에 이 문제를 처리할 방법을 알아볼 것이다.
