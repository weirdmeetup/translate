...
당신이 소프트웨어 개발자라면 말이다. 모나드(monads)에 관해 들어봤거나 무엇인지 궁금했던 적이 있는가? 하스켈을 배우려고 시도해보고 그 어려움에 고생해본 적이 있는가? 사람들이 왜 이런 것들을 고민하는지 의문이 들지 않았나? 마이크로소프트 "Channel 9"에서 마이크로소프트 사람들이 이야기하는 것은 들어봤지만 개발을 매일 하면서도 관련된 문제를 해결해본 적이 없었을 지도 모른다.
...
### 함수로 실패 다루기
의사(pseudo) 함수로 작성하는 첫 예제는 실패할 수 있는 함수다. 함수가 실패할 것이란 사실을 아는 것은 크게 어려운 점이 아니다. 다음 두가지의 결과를 포함하고 있는 함수를 의미한다.
